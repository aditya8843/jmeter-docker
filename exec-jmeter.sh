...
WDIR=`docker exec -it master /bin/bash`
mkdir -p results
for filename in sample-test/*.jmx; do
    NAME=$(basename $filename)
    NAME="${NAME%.*}"
    eval "docker cp $filename master:$WDIR/sample-test/"
    eval "docker exec -it master /bin/bash -c 'mkdir $NAME && cd $NAME && ../jmeter -n -t ../$filename -R$SLAVE_IP'"
